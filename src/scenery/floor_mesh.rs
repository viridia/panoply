...
use super::precinct::{Precinct, PrecinctAssetChanged};

/// Spawns a task for each parcel to compute the water mesh geometry.
pub fn gen_floor_meshes(
    mut commands: Commands,
    mut query: Query<(Entity, &mut Precinct), With<PrecinctAssetChanged>>,
    ...
    let _pool = AsyncComputeTaskPool::get();

    for (entity, _precinct) in query.iter_mut() {
        // let realm = realms_query.get(precinct.realm);
        // if realm.is_err() {
        //     return;
        // }
        ...
        // let shape_refs = precinct.contours;
...
// pub fn insert_floor_meshes(
//     mut commands: Commands,
//     mut query: Query<(Entity, &mut Precinct, &mut ComputeWaterMeshTask)>,
//     realms_query: Query<(&Realm, &TerrainMap)>,
